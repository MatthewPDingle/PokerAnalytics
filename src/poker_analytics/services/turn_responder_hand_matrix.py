"""Aggregate responder hand classifications by turn bet-size bucket."""
...
def _aggregate(events: Iterable[Mapping[str, object]]) -> dict:
    ...
    scenarios: List[dict] = []
    for (hero_position, bet_line, position, player_count, response_type, texture_key, preflop_key), bucket_map in sorted(
        scenario_map.items(),
        key=lambda item: (
...
__all__ = ["load_turn_responder_hand_matrix"]
CURRENT_VERSION = 1
