#!/usr/bin/env python3
"""Build PokerStars NL10 river hand matrix cache from raw HH text files.

This script parses PokerStars NL10 hand histories under a given root
directory (default: ``data/NL10``), extracts river bet events where the
bettor's hole cards are revealed at showdown, and aggregates them using
the existing ``river_hand_matrix`` aggregator.

The resulting payload is written to:

    var/cache/pokerstars_nl10/river_hand_matrix_<stake_token>.json
...
from poker_analytics.importers.pokerstars_text import collect_pokerstars_river_events
from poker_analytics.services.river_hand_matrix import _aggregate as aggregate_river_hands, CURRENT_VERSION
...
def main(argv: list[str] | None = None) -> int:
  ...
  events = collect_pokerstars_river_events(root, stake_policy=stake_policy)
  if not events:
      print("No eligible PokerStars river events found; nothing to write.")
      return 0

  payload = aggregate_river_hands(events)
  ...
  print(f"PokerStars river hand matrix written to {destination}")
  return 0
