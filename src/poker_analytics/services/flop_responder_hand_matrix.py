...
def load_flop_responder_hand_matrix(source: str | None = None) -> dict:
    """Return aggregated responder hand distributions by flop bet sizing."""

    stake_policy = StakePolicy.from_environment()
    data_paths = build_data_paths()
    filename = f"flop_responder_hand_matrix_{stake_policy.cache_token()}.json"
...
def _aggregate(events: Iterable[Mapping[str, object]]) -> dict:
    ...
    for event in events:
        ...
        for response in responses:
            ...
            # Map missing/unrecognised hand types into an \"Unknown\" bucket so
            # responder event counts include every response, even when we do
            # not have a detailed strength label for the hand.
