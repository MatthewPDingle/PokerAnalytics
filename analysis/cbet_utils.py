...
def _straight_info(
    hole: List[Tuple[str, int, str]],
    board: List[Tuple[str, int, str]],
    ...
    for candidate in range(1, 15):
        ...
        if not _has_straight(augmented):
            continue
        # Determine whether the candidate completes the straight as an end card or an inside card
        treated = False
        for start in range(1, 11):
            seq = [start + offset for offset in range(5)]
...
def _classify_hand(hole: List[Tuple[str, int, str]], board: List[Tuple[str, int, str]]) -> Dict[str, bool | str]:
    ...
    straight, oesd, double_gutter = _straight_info(hole, board)
    ...
    return {
        ...
        "oesd": oesd or double_gutter,
        "oesd_dg": oesd or double_gutter,
...
def _validate_primary_groups(primary_groups: Dict[str, Sequence[str]], available_categories: Iterable[str]) -> None:
    ...
    recognised = set(BASE_PRIMARY_CATEGORIES)
    ...
    for label, members in primary_groups.items():
        ...
        for cat in members:
            if cat not in recognised:
                unknown.append((label, cat))
            elif cat not in available:
                absent.add(cat)

    if unknown:
        options = ", ".join(sorted(recognised))
        details = ", ".join(f"{label}:{cat}" for label, cat in unknown)
        raise ValueError(f"Unknown categories in PRIMARY_GROUPS: {details}. Allowed: {options}")
    ...
    if absent:
        print(f"Note: categories without events in this dataset: {', '.join(sorted(absent))}")
                    ...
                    if (
                        ...
                        if response_kind:
                            ...
                            if flop_cards and player in pocket_cards:
                                responder_class = _classify_hand(pocket_cards[player], flop_cards)
            ...
            response_rows.append(
                {
                    ...
                    "is_all_in_cbet": bool(event.get("is_all_in")),
                    "is_one_bb_cbet": bool(event.get("is_one_bb")),
                }
            )
...
def parse_cards_text(text: str | None) -> List[Tuple[str, int, str]]:
    """Public wrapper for parsing Ignition card strings."""

    return _parse_cards(text)


def classify_hand(hole_cards: Sequence[Tuple[str, int, str]], board_cards: Sequence[Tuple[str, int, str]]) -> Dict[str, bool | str]:
    """Classify a player's combo against the provided board cards."""

    return _classify_hand(list(hole_cards), list(board_cards))


def extract_big_blind(root: ET.Element) -> float | None:
    """Public wrapper exposing big-blind extraction from a parsed hand."""

    return _extract_big_blind(root)
