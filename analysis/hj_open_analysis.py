#!/usr/bin/env python3
"""Analyse HJ (MP in 6-max) open sizing and downstream responses."""
...
def analyse(hero_seat, positions, bb_map, actions_map):
    ...
    for hand_id, acts in actions_map.items():
        ...
        for seat_no in {seat for seat, _, _, _ in acts}:
            if positions.get((hand_id, seat_no)) == target_pos:
                hj_seat = seat_no
                break
        if hj_seat is None:
            continue
...
def summarize(events):
    ...
    _summary("Hero HJ opens", hero_events)
    _summary("Population HJ opens", pop_events)
...
def main() -> None:
    ...
    try:
        ...
        print(f"Total HJ opens captured: {len(events)}")
        summarize(events)
    finally:
        conn.close()
