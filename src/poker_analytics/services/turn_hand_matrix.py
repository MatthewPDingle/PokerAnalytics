...
def _load_cached_payload(cache_dir: Path, filename: str, *, source: str | None = None) -> dict | None:
    ...
    When a concrete ``source`` key is provided (e.g. ``pokerstars_nl10``) we
    only read from the corresponding subdirectory. This avoids accidentally
    falling back to the default DriveHUD cache when a per-source cache has not
    been generated yet.
    """
...
def load_turn_hand_matrix(source: str | None = None) -> dict:
...
def _aggregate(events: Iterable[Mapping[str, object]]) -> dict:
    scenario_map: Dict[Tuple[str, str, str, int, str, str, str], Dict[str, Dict[str, object]]] = {}
