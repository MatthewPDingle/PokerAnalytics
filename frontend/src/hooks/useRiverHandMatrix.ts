...
export const useRiverHandMatrix = (sourceKey: string | null): HandMatrixState => {
      ...
      try {
        const url =
          sourceKey && sourceKey.trim().length > 0
            ? `/api/river/hand-types?source=${encodeURIComponent(sourceKey)}`
            : '/api/river/hand-types';
        ...
        console.error('Failed to load river hand matrix', err);
        setState({
  ...
  textures: [
    ...
    { key: 'two_tone', label: 'Two Tone Rivers' },
          ...
          loading: false,
          error: 'Unable to load river hand breakdown. Displaying sample data.',
          usingSample: true,
        });
      }
    };
...
export default useRiverHandMatrix;
