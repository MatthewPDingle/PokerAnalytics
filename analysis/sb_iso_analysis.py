#!/usr/bin/env python3
"""Analyse SB raises over limpers (iso-limp) for the entire population."""
...
def analyse_hand(
    ...
    seat = raiser["seat"]
    if seat_positions.get(seat) != "SB":
        return None

    # Ensure no raises before
    prior_actions = actions[:first_raise_idx]
    limpers = 0
    for act in prior_actions:
        ...
        if act["action"] == "call":
            limpers += 1
    if limpers == 0:
        return None  # need at least one limper
...
def summarize(title: str, attempts: List[IsoAttempt]) -> None:
    if not attempts:
        ...
        return
    avg_size = sum(a.raise_size_bb for a in attempts) / len(attempts)
    ...
    size_counter = Counter(round(a.raise_size_bb, 2) for a in attempts)
    for att in attempts:
    ...
    for size, count in top_sizes:
        succ = sum(1 for a in attempts if round(a.raise_size_bb, 2) == size and a.success)
...
def main() -> None:
    ...
    try:
        ...
        summarize("Hero SB iso raises", hero_attempts)
        summarize("Population SB iso raises", population_attempts)

        summarize("Hero SB iso raises (1 limper)", filter_by_limpers(hero_attempts, 1, 1))
        summarize("Hero SB iso raises (>=2 limpers)", filter_by_limpers(hero_attempts, 2, None))

        summarize("Population SB iso raises (1 limper)", filter_by_limpers(population_attempts, 1, 1))
        summarize("Population SB iso raises (>=2 limpers)", filter_by_limpers(population_attempts, 2, None))
    finally:
        conn.close()
