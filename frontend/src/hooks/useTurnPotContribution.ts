...
import { TurnBucketMeta, TurnResponseScenario, SelectOption } from './useTurnResponseMatrix';
...
const SAMPLE_BUCKETS: TurnBucketMeta[] = [
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.error('Failed to load turn pot contribution', error);
        if (!cancelled) {
          setState({
...
export default useTurnPotContribution;
