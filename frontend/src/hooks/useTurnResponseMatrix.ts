...
export type TurnBucketMeta = {
  key: string;
  label: string;
};

export type TurnResponseMetric = {
  bucketKey: string;
  bucketLabel: string;
  events: number;
  foldEvents: number;
  ...
  raiseEvents: number;
  avgRatio: number;
  avgAddedTurnBb: number;
  avgAddedAllBb: number;
  avgShareAll: number;
  avgBreakevenPct: number;
};

export type TurnResponseScenario = {
  heroPosition: string;
  betLine: string;
  position: 'IP' | 'OOP';
  playerCount: number;
...
const SAMPLE_BUCKET_ORDER: TurnBucketMeta[] = [
...
const SAMPLE_SCENARIOS: TurnResponseScenario[] = [
  {
    heroPosition: 'BTN',
...
type HookState = {
  data: TurnResponseScenario[];
  bucketOrder: TurnBucketMeta[];
...
const transformPayload = (payload: RawPayload): HookState => {
  const scenarios: TurnResponseScenario[] = payload.scenarios.map((scenario) => ({
    ...
    metrics: scenario.metrics.map((metric) => ({
      ...
      avgAddedTurnBb: metric.avg_added_turn_bb ?? 0,
...
const SAMPLE_STATE: HookState = {
  ...
  loading: false,
  error: 'Using sample turn response data (API unavailable).',
  usingSample: true,
};

export const useTurnResponseMatrix = (sourceKey: string | null) => {
      ...
      try {
        const url =
          sourceKey && sourceKey.trim().length > 0
            ? `/api/turn/response-matrix?source=${encodeURIComponent(sourceKey)}`
            : '/api/turn/response-matrix';
