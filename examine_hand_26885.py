...
def main():
    ...
    row = list(source.rows('SELECT HandHistoryId, HandNumber, HandHistory FROM HandHistories WHERE HandHistoryId = 26885'))[0]
    ...
    for game in session.findall('game'):
        ...
        for p in sorted(players, key=lambda x: x['seat']):
            ...
            print(f"  Seat {p['seat']}: {p['name']}{dealer_mark}{sb_mark}{bb_mark}{hero_mark} - {cards}")
