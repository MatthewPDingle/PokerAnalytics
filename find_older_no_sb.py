#!/usr/bin/env python3
"""Find older hands where no SB was posted."""
...
def main():
    ...
    # Get ALL hands in normal order (oldest first)
    ...
    if candidates:
        print(f"Found {len(candidates)} total hands where SB wasn't posted and Hero would be SB\n")
        print("="*70)
        print("OLDEST 10 HANDS:")
        print("="*70)

        # Show the first 10 (oldest)
        ...
        print("\n" + "="*70)
        print("MIDDLE 10 HANDS (around hand 60-70):")
        print("="*70)

        # Show middle range
        start_idx = min(60, len(candidates) - 10)
        for i, hand in enumerate(candidates[start_idx:start_idx+10], start_idx+1):
            print(f"\n{i}. Hand Option:")
            print(f"   HandHistoryId: {hand['hand_id']}")
            print(f"   HandNumber: {hand['hand_number']}")
            print(f"   Date/Time: {hand['start_date']}")
