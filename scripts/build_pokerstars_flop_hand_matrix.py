#!/usr/bin/env python3
"""Build PokerStars NL10 flop hand matrix cache from raw HH text files.

This script parses PokerStars NL10 hand histories under a given root
directory (default: ``data/NL10``), extracts flop bet events where the
bettor's hole cards are revealed at showdown, and aggregates them using
the existing ``flop_hand_matrix`` aggregator.

The resulting payload is written to:
    var/cache/pokerstars_nl10/flop_hand_matrix_<stake_token>.json
...
from poker_analytics.importers.pokerstars_text import collect_pokerstars_flop_events
from poker_analytics.services.flop_hand_matrix import _aggregate as aggregate_flop_hands, CURRENT_VERSION
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional cap on the number of files to scan (for smoke tests).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    events = collect_pokerstars_flop_events(root, stake_policy=stake_policy)

    if not events:
        print("No eligible PokerStars flop events found; nothing to write.")
        return 0

    payload = aggregate_flop_hands(events)
    ...
    print(f"PokerStars flop hand matrix written to {destination}")
    return 0
