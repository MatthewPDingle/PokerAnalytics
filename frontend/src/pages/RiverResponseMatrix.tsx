...
import {
  SelectOption,
  RiverResponseScenario,
  useRiverResponseMatrix,
} from '../hooks/useRiverResponseMatrix';
import useRiverHandMatrix from '../hooks/useRiverHandMatrix';
import useRiverResponderHandMatrix from '../hooks/useRiverResponderHandMatrix';
...
const combineScenarios = (scenarios: RiverResponseScenario[]) => {
  ...
  scenarios.forEach((scenario) => {
    scenario.metrics.forEach((metric) => {
      ...
      aggregates.set(metric.bucketKey, {
        ...
        addedRiverSum: existing.addedRiverSum + (metric.avgAddedRiverBb ?? 0) * metric.events,
...
const RiverResponseMatrix = () => {
  ...
  const {
    ...
    error,
    usingSample,
  } = useRiverResponseMatrix(activeSourceKey);
  ...
  } = useRiverHandMatrix(activeSourceKey);
  ...
  } = useRiverResponderHandMatrix(activeSourceKey);
  ...
  const textureOptions = useMemo(() => {
    ...
    return [
      ...
      { key: 'three_tone', label: 'Three Tone Rivers' },
      { key: 'four_tone', label: 'Four Tone Rivers' },
      { key: 'connected_le7', label: 'Connected (≤7 Gap)' },
      { key: 'three_connected', label: '3 Connected Ranks' },
      { key: 'four_connected', label: '4 Connected Ranks' },
      { key: 'paired', label: 'Paired River' },
      { key: 'double_paired', label: 'Double Paired River' },
      { key: 'trips', label: 'Trips On Board' },
      { key: 'ace_high', label: 'Ace-High River' },
      { key: 'low', label: 'Low Rivers (All ≤ Ten)' },
      { key: 'high', label: 'High Rivers (≥4 Tens+)' },
    ];
  }, [textures]);
...
export default RiverResponseMatrix;
