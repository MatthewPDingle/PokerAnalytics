...
export type RiverBucketMeta = {
  key: string;
  label: string;
};

export type RiverResponseMetric = {
  bucketKey: string;
  bucketLabel: string;
  events: number;
  foldEvents: number;
  ...
  raiseEvents: number;
  avgRatio: number;
  avgAddedRiverBb: number;
  avgAddedAllBb: number;
  avgShareAll: number;
  avgBreakevenPct: number;
};

export type RiverResponseScenario = {
  heroPosition: string;
  betLine: string;
  position: 'IP' | 'OOP';
  playerCount: number;
...
const SAMPLE_BUCKET_ORDER: RiverBucketMeta[] = [
...
const SAMPLE_SCENARIOS: RiverResponseScenario[] = [
  {
    heroPosition: 'BTN',
...
const transformPayload = (payload: RawPayload): HookState => {
  const scenarios: RiverResponseScenario[] = payload.scenarios.map((scenario) => ({
    ...
    metrics: scenario.metrics.map((metric) => ({
      ...
      avgAddedRiverBb: metric.avg_added_river_bb ?? 0,
