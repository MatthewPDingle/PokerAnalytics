...
const frequencyGradient = (value: number, max: number): ColorResult => {
  if (!Number.isFinite(value) || max < EPSILON) {
    return toColorResult(WHITE);
  }

  const ratio = clampRatio(Math.max(0, value) / max);
  return toColorResult(mix(WHITE, BLUE, ratio) as Rgb);
};
...
const RangeGrid = ({ rows, cols, values, variant = 'equity' }: RangeGridProps) => {
  const { min, max } = calculateBounds(values);
  const frequencyMax = Math.max(max, 0);
          ...
          <Tr key={row}>
            ...
            {cols.map((col, j) => {
              ...
              const { background, textColor } =
                variant === 'frequency'
                  ? frequencyGradient(value, frequencyMax)
                  : anchoredGradient(value, min, max, 50);
              return (
