...
export const useTurnResponderHandMatrix = (sourceKey: string | null) => {
      ...
      try {
        const url =
          sourceKey && sourceKey.trim().length > 0
            ? `/api/turn/responder-hand-matrix?source=${encodeURIComponent(sourceKey)}`
            : '/api/turn/responder-hand-matrix';
