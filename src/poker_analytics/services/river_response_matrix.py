...
def load_river_response_matrix() -> dict:
    """Return the aggregated payload used by the frontend heatmap."""

    stake_policy = StakePolicy.from_environment()
    data_paths = build_data_paths()
    cache_path = data_paths.cache_dir / f"river_response_matrix_{stake_policy.cache_token()}.json"
...
def load_river_pot_contribution() -> dict:
    """Return average pot contribution per bet-size bucket for river bets."""

    stake_policy = StakePolicy.from_environment()
    data_paths = build_data_paths()
    cache_path = data_paths.cache_dir / f"river_pot_contribution_{stake_policy.cache_token()}.json"
