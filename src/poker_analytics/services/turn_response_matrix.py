"""Aggregate villain responses to turn bets by bet size."""
...
BETTING_LINE_OPTIONS: Sequence[Mapping[str, str]] = (
    ...
    {"key": "delayed_cbet", "label": "Delayed C-Bet (X;B)"},
    {"key": "probe", "label": "Probe (X-X;B)"},
    {"key": "xr_barrel", "label": "XR Barrel (XR;B)"},
    {"key": "raise_barrel", "label": "Raise Barrel (R;B)"},
...
def build_turn_response_payload(events: Iterable[Mapping[str, object]]) -> dict:
...
def _aggregate_events(events: Iterable[Mapping[str, object]]) -> Tuple[List[dict], List[int], List[str], set[str], set[str], set[str]]:
    ...
    for event in events:
        ...
        if player_count_raw is None:
            player_count_raw = event.get("turn_players")
        try:
        ...
        total_turn_bb = _safe_float(event.get("total_added_turn_bb"))
        total_all_bb = _safe_float(event.get("total_added_all_bb")) or total_turn_bb
                ...
                "metrics": [
                    {
                        ...
                        "avg_added_turn_bb": (
                            bucket_map[meta.key]["total_added_turn_bb"] / bucket_map[meta.key]["events"]
                            if bucket_map[meta.key]["events"]
                            else 0.0
                        ),
                        "avg_added_all_bb": (
...
def _event_texture_keys(event: Mapping[str, object]) -> list[str]:
    ...
    if isinstance(raw, (list, tuple, set)):
        ...
        if textures:
            return textures
    turn_text = event.get("turn_cards") or event.get("board_turn")
    return turn_texture_keys(turn_text)
...
def build_turn_pot_contribution_payload(events: Iterable[Mapping[str, object]]) -> dict:
